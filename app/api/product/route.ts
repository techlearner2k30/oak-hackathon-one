...
import { productsTable , products , insertProducts , db} from '@/lib/drizzle'
   ...
   //console.log(db);
    try {
       // const client = await db.connect();
        await sql`create TABLE IF NOT EXISTS Products(id SERIAL,name VARCHAR(255), description TEXT,imgurl VARCHAR(255), price money, sizeid int);`
        const res1 = await db.select().from(productsTable);
        console.log(res1[5].name);
       ...
       // console.log(json.parse(rowData.rows[0].sizes));
        return NextResponse.json({ data: res1 });
        //console.log(client);
    } catch (err) {
        console.log(err)
...
export async function POST(request: NextRequest) {
    ...
    const insertProducts:insertProducts = {
        name : "Owais Ahmed 12345678123",
    ...
    try {
        if (req.name) {
            ...
            const abc = await db.insert(productsTable).values(insertProducts).returning();
            console.log(abc);
